...
class ENCODEChIP:	
	...
	def nodupbam(self, base, ctl=False):
		...
		pattern = '{0}/{1}/*/glob*/*nodup.bam'.format(base, tag)
	...
	def files(self, base):
		...
		hashed['qc.html'] = glob.glob('{0}/call-qc_report/glob*/qc.html'.format(base))
		hashed['qc.json'] = glob.glob('{0}/call-qc_report/glob*/qc.json'.format(base))
		hashed['pval.signal.bigwig'] = glob.glob('{0}/call-macs2/*/glob*/*pval.signal.bigwig'.format(base))
		hashed['fc.signal.bigwig'] = glob.glob('{0}/call-macs2/*/glob*/*fc.signal.bigwig'.format(base))
		hashed['pval0.01.500K.narrowPeak.gz'] = glob.glob('{0}/call-macs2/*/glob*/*merged.nodup_x_ctl_for_rep1*pval0.01.500K.narrowPeak.gz'.format(base))
		hashed['pval0.01.500K.bfilt.narrowPeak.gz'] = glob.glob('{0}/call-macs2/*/glob*/*merged.nodup_x_ctl_for_rep1.pval0.01.500K.bfilt.narrowPeak.gz'.format(base))
		return hashed
	...
	def postprocess(self, cromwell, ctl, pet=True):
		...
		return self.postprocessing_tasks(bam, ctl=ctl, pet=pet, fragment=fragment)

	def postprocessing_tasks(self, bam, ctl, pet, fragment):
		out = dict()
